...
- `python/format` 只接受 1~2 参数，格式化字符串请用 `(. "..." format ...)`。
- `set!` 不能用于普通局部变量累加；用 `loop/recur` 累积（例如计时/求和）。
- `try/catch/finally` 顺序必须是 `try ... (catch ...) (finally ...)`。
...
- 信号发射要写成 `(. (.-signal obj) emit ...)`，不要写 `(. obj signal emit ...)`。
- `python/classmethod` 本身不可调用：直接 `(python/classmethod fn)` 放入 `python/type` dict。
- QML 例子建议从仓库根目录运行；QML 路径引用了 `python/` 目录下的 `.qml` 文件。
- `QMainWindow` 必须 `setCentralWidget`；`QWidget` 才用 `QVBoxLayout window`。
...
- `TypeError: 'PosixPath' object is not callable` → 属性用 `.-` 取，不要 `(. obj attr)`。
- QML 加载失败时，检查工作目录和 `import` 版本是否被替换成 `PythonCounter 1.0`。
